...
```
public/         the frontend — index.html, nexus-register.html, nexus-dashboard-mobile.html, served statically by Express
src/
...
| `index.html` (phone/email + password) | `POST`, sets httpOnly refresh cookie | `POST /api/v1/auth/login` |
| `index.html` forgot-password modal | `POST`, then `POST` with the emailed/texted token | `POST /api/v1/auth/forgot-password`, `POST /api/v1/auth/reset-password` |
